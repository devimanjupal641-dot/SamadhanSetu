...
export interface Problem {
  ...
  embedding_vector?: number[]; // MiniLM 384-dim or dense semantic vector
  district: string;
  ward: string;
  state: string;
  latitude: number;
  ...
  cluster_id?: string;
  severity_score: number; // 1-100 computed from citizen reports, urgency, geographic density
...
export interface ProblemCluster {
  ...
  ward: string;
  report_count: number;
  representative_problem_id: string;
  created_at: string;
  updated_at: string;
}
